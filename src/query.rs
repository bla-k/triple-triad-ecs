...
//use crate::game::{Cursor, Entity, InteractionCtx, Player};
//
//pub fn can_place_card(ctx: &InteractionCtx, positions: &[Option<Cursor>]) -> bool {
//    // cannot place a card that is not in hand
//    if !matches!(ctx.selection, Some(Cursor::Hand(_))) {
//        return false;
//    }
//
//    // cannot place a card without a destination on the board
//    let Some(cursor) = &ctx.cursor else {
//        return false;
//    };
//
//    // cannot place a card on a cell that is already occupied
//    positions
//        .iter()
//        .any(|position| !matches!(position.as_ref(), Some(position) if position == cursor))
//}
//
//pub fn get_hand_entity(
//    player: Player,
//    position: &Cursor,
//    owners: &[Option<Player>],
//    positions: &[Option<Cursor>],
//) -> Option<Entity> {
//    (0..positions.len()).find_map(|entity| {
//        if owners[entity] == Some(player) && positions[entity].as_ref() == Some(position) {
//            Some(entity)
//        } else {
//            None
//        }
...
//pub fn hand_size(player: Player, owners: &[Option<Player>], positions: &[Option<Cursor>]) -> usize {
//    (0..owners.len())
//        .filter(|&entity| {
//            owners[entity] == Some(player) && matches!(positions[entity], Some(Cursor::Hand(_)))
//        })
//        .count()
//}
